...
namespace NFPConnect.Droid.Services
{
    public class BiometricAuthService : IBiometricAuthenticateService
    {
                    ...
                    if (fingerprintManager != null && fingerprintManager.IsHardwareDetected)
                    {
                        if (keyguardManager.IsKeyguardSecure)
                        {
                            ...
                            else
                            {
                                ...
                                Log.Error("P keyGuardEnabled", "User not enrolled any fingerprints to authenticate");
        ...
        public void triggerKeyGuardPage()
        {
            KeyguardManager keyguardManager = (KeyguardManager)context.GetSystemService(Context.KeyguardService);
            var intent = keyguardManager.CreateConfirmDeviceCredentialIntent((string)null, (string)null);
            if (intent != null)
            {
                MainActivity.FormsContext.StartActivityForResult(intent, MainActivity.ConfirmRequestId);
            }
        }
    }
}